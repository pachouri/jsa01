...
export class RegionService {
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/regions");
  ...
  update(region: IRegion): Observable<EntityResponseType> {
    return this.http.put<IRegion>(
      `${this.resourceUrl}/${getRegionIdentifier(region) as number}`,
      region,
      { observe: "response" }
    );
  }
  ...
  partialUpdate(region: IRegion): Observable<EntityResponseType> {
    return this.http.patch<IRegion>(
      `${this.resourceUrl}/${getRegionIdentifier(region) as number}`,
      region,
      { observe: "response" }
    );
  ...
  addRegionToCollectionIfMissing(
    regionCollection: IRegion[],
    ...regionsToCheck: (IRegion | null | undefined)[]
  ): IRegion[] {
    const regions: IRegion[] = regionsToCheck.filter(isPresent);
    if (regions.length > 0) {
      const regionCollectionIdentifiers = regionCollection.map(
        (regionItem) => getRegionIdentifier(regionItem)!
      );
      const regionsToAdd = regions.filter((regionItem) => {
        const regionIdentifier = getRegionIdentifier(regionItem);
        if (
          regionIdentifier == null ||
          regionCollectionIdentifiers.includes(regionIdentifier)
        ) {
          return false;
        }
