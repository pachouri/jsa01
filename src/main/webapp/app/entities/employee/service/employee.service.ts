...
export class EmployeeService {
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/employees");
  ...
  update(employee: IEmployee): Observable<EntityResponseType> {
    ...
    return this.http
      .put<IEmployee>(
        `${this.resourceUrl}/${getEmployeeIdentifier(employee) as number}`,
        copy,
        { observe: "response" }
      )
      .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http
      .get<IEmployee[]>(this.resourceUrl, {
        params: options,
        observe: "response",
      })
      .pipe(
  ...
  addEmployeeToCollectionIfMissing(
    employeeCollection: IEmployee[],
    ...employeesToCheck: (IEmployee | null | undefined)[]
  ): IEmployee[] {
    const employees: IEmployee[] = employeesToCheck.filter(isPresent);
    if (employees.length > 0) {
      const employeeCollectionIdentifiers = employeeCollection.map(
        (employeeItem) => getEmployeeIdentifier(employeeItem)!
      );
      const employeesToAdd = employees.filter((employeeItem) => {
        const employeeIdentifier = getEmployeeIdentifier(employeeItem);
        if (
          employeeIdentifier == null ||
          employeeCollectionIdentifiers.includes(employeeIdentifier)
        ) {
          return false;
        }
  ...
  protected convertDateFromClient(employee: IEmployee): IEmployee {
    return Object.assign({}, employee, {
      hireDate: employee.hireDate?.isValid()
        ? employee.hireDate.toJSON()
        : undefined,
    });
  }
