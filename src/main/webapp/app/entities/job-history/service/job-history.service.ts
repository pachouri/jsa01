...
export class JobHistoryService {
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/job-histories");
  ...
  update(jobHistory: IJobHistory): Observable<EntityResponseType> {
    ...
    return this.http
      .put<IJobHistory>(
        `${this.resourceUrl}/${getJobHistoryIdentifier(jobHistory) as number}`,
        copy,
        { observe: "response" }
      )
      .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http
      .get<IJobHistory[]>(this.resourceUrl, {
        params: options,
        observe: "response",
      })
      .pipe(
  ...
  addJobHistoryToCollectionIfMissing(
    ...
    if (jobHistories.length > 0) {
      const jobHistoryCollectionIdentifiers = jobHistoryCollection.map(
        (jobHistoryItem) => getJobHistoryIdentifier(jobHistoryItem)!
      );
      const jobHistoriesToAdd = jobHistories.filter((jobHistoryItem) => {
        ...
        if (
          jobHistoryIdentifier == null ||
          jobHistoryCollectionIdentifiers.includes(jobHistoryIdentifier)
        ) {
          return false;
        }
  ...
  protected convertDateFromClient(jobHistory: IJobHistory): IJobHistory {
    return Object.assign({}, jobHistory, {
      startDate: jobHistory.startDate?.isValid()
        ? jobHistory.startDate.toJSON()
        : undefined,
      endDate: jobHistory.endDate?.isValid()
        ? jobHistory.endDate.toJSON()
        : undefined,
    });
  }
    ...
    if (res.body) {
      res.body.forEach((jobHistory: IJobHistory) => {
        jobHistory.startDate = jobHistory.startDate
          ? dayjs(jobHistory.startDate)
          : undefined;
        jobHistory.endDate = jobHistory.endDate
          ? dayjs(jobHistory.endDate)
          : undefined;
      });
    }
    return res;
