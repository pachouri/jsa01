...
export class LocationService {
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/locations");
  ...
  update(location: ILocation): Observable<EntityResponseType> {
    return this.http.put<ILocation>(
      `${this.resourceUrl}/${getLocationIdentifier(location) as number}`,
      location,
      { observe: "response" }
    );
  }
  ...
  partialUpdate(location: ILocation): Observable<EntityResponseType> {
    return this.http.patch<ILocation>(
      `${this.resourceUrl}/${getLocationIdentifier(location) as number}`,
      location,
      { observe: "response" }
    );
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http.get<ILocation>(`${this.resourceUrl}/${id}`, {
      observe: "response",
    });
  }
  ...
  addLocationToCollectionIfMissing(
    locationCollection: ILocation[],
    ...locationsToCheck: (ILocation | null | undefined)[]
  ): ILocation[] {
    const locations: ILocation[] = locationsToCheck.filter(isPresent);
    if (locations.length > 0) {
      const locationCollectionIdentifiers = locationCollection.map(
        (locationItem) => getLocationIdentifier(locationItem)!
      );
      const locationsToAdd = locations.filter((locationItem) => {
        const locationIdentifier = getLocationIdentifier(locationItem);
        if (
          locationIdentifier == null ||
          locationCollectionIdentifiers.includes(locationIdentifier)
        ) {
          return false;
        }
