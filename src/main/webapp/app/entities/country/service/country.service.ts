...
export class CountryService {
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/countries");
  ...
  update(country: ICountry): Observable<EntityResponseType> {
    return this.http.put<ICountry>(
      `${this.resourceUrl}/${getCountryIdentifier(country) as number}`,
      country,
      { observe: "response" }
    );
  }
  ...
  partialUpdate(country: ICountry): Observable<EntityResponseType> {
    return this.http.patch<ICountry>(
      `${this.resourceUrl}/${getCountryIdentifier(country) as number}`,
      country,
      { observe: "response" }
    );
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http.get<ICountry>(`${this.resourceUrl}/${id}`, {
      observe: "response",
    });
  }
  ...
  addCountryToCollectionIfMissing(
    countryCollection: ICountry[],
    ...countriesToCheck: (ICountry | null | undefined)[]
  ): ICountry[] {
    const countries: ICountry[] = countriesToCheck.filter(isPresent);
    if (countries.length > 0) {
      const countryCollectionIdentifiers = countryCollection.map(
        (countryItem) => getCountryIdentifier(countryItem)!
      );
      const countriesToAdd = countries.filter((countryItem) => {
        ...
        if (
          ...
          countryCollectionIdentifiers.includes(countryIdentifier)
        ) {
          return false;
        }
