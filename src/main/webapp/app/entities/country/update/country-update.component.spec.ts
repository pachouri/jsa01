  ...
  describe("Country Management Update Component", () => {
    ...
    describe("ngOnInit", () => {
      it("Should call region query and add missing value", () => {
        ...
        jest
          .spyOn(regionService, "query")
          .mockReturnValue(of(new HttpResponse({ body: regionCollection })));
        const expectedCollection: IRegion[] = [region, ...regionCollection];
        jest
          .spyOn(regionService, "addRegionToCollectionIfMissing")
        ...
        expect(
          regionService.addRegionToCollectionIfMissing
        ).toHaveBeenCalledWith(regionCollection, region);
