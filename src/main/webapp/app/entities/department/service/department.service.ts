...
export class DepartmentService {
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/departments");
  ...
  update(department: IDepartment): Observable<EntityResponseType> {
    return this.http.put<IDepartment>(
      `${this.resourceUrl}/${getDepartmentIdentifier(department) as number}`,
      department,
      { observe: "response" }
    );
  }
  ...
  addDepartmentToCollectionIfMissing(
    ...
    if (departments.length > 0) {
      const departmentCollectionIdentifiers = departmentCollection.map(
        (departmentItem) => getDepartmentIdentifier(departmentItem)!
      );
      const departmentsToAdd = departments.filter((departmentItem) => {
        ...
        if (
          ...
          departmentCollectionIdentifiers.includes(departmentIdentifier)
        ) {
          return false;
        }
