...
export class JobService {
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/jobs");
  ...
  update(job: IJob): Observable<EntityResponseType> {
    return this.http.put<IJob>(
      `${this.resourceUrl}/${getJobIdentifier(job) as number}`,
      job,
      { observe: "response" }
    );
  }
  ...
  addJobToCollectionIfMissing(
    jobCollection: IJob[],
    ...jobsToCheck: (IJob | null | undefined)[]
  ): IJob[] {
    const jobs: IJob[] = jobsToCheck.filter(isPresent);
    if (jobs.length > 0) {
      const jobCollectionIdentifiers = jobCollection.map(
        (jobItem) => getJobIdentifier(jobItem)!
      );
      const jobsToAdd = jobs.filter((jobItem) => {
        const jobIdentifier = getJobIdentifier(jobItem);
        if (
          jobIdentifier == null ||
          jobCollectionIdentifiers.includes(jobIdentifier)
        ) {
          return false;
        }
